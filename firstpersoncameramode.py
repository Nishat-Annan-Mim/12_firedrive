...
GAME_TIME_LIMIT = 18 # 3 minutes in seconds
...
def draw_mountain(x, y, z, height, width):
    """Draw a mountain using triangular faces"""
    glPushMatrix()
    glTranslatef(x, y, z)
    
    # Mountain base color (dark gray)
    glColor3f(0.4, 0.4, 0.4)
    
    # Draw mountain as multiple triangular faces
    base_size = width
    
    # Front face
    glBegin(GL_TRIANGLES)
    glVertex3f(0, height, 0)  # Peak
    glVertex3f(-base_size/2, 0, base_size/2)  # Front left base
    glVertex3f(base_size/2, 0, base_size/2)   # Front right base
    glEnd()
    
    # Back face
    glBegin(GL_TRIANGLES)
    glVertex3f(0, height, 0)  # Peak
    glVertex3f(base_size/2, 0, -base_size/2)   # Back right base
    glVertex3f(-base_size/2, 0, -base_size/2)  # Back left base
    glEnd()
    ...
    # Base (square)
    glColor3f(0.3, 0.3, 0.3)  # Darker base
    glBegin(GL_QUADS)
    ...
    glColor3f(1.0, 1.0, 1.0)
    peak_size = width * 0.3
    peak_start = height * 0.7
    
    # Front snow face
    glBegin(GL_TRIANGLES)
    glVertex3f(0, height, 0)
    glVertex3f(-peak_size/2, peak_start, peak_size/2)
    glVertex3f(peak_size/2, peak_start, peak_size/2)
    glEnd()
...
def draw_tree(x, y, z, trunk_height, crown_size):
    ...
    glutSolidSphere(crown_size/2, 12, 12)
    glPopMatrix()
...
def draw_pit_holes():
    ...
    for pit in pit_holes:
        ...
        # --- Draw filled circle with rainbow ---
        for i in range(num_segments):
            angle1 = 2 * math.pi * i / num_segments
            angle2 = 2 * math.pi * (i + 1) / num_segments

            r = 0.5 + 0.5 * math.sin(angle1)
            g = 0.5 + 0.5 * math.sin(angle1 + 2 * math.pi / 3)
            b = 0.5 + 0.5 * math.sin(angle1 + 4 * math.pi / 3)
            ...
            glColor3f(r, g, b)
            glVertex3f(math.cos(angle1) * PIT_RADIUS, 0, math.sin(angle1) * PIT_RADIUS)

            r2 = 0.5 + 0.5 * math.sin(angle2)
            g2 = 0.5 + 0.5 * math.sin(angle2 + 2 * math.pi / 3)
            b2 = 0.5 + 0.5 * math.sin(angle2 + 4 * math.pi / 3)
            glColor3f(r2, g2, b2)
            glVertex3f(math.cos(angle2) * PIT_RADIUS, 0, math.sin(angle2) * PIT_RADIUS)
            glEnd()
        ...
        for i in range(num_segments):
            ...
            glBegin(GL_LINES)
            glColor3f(r, g, b)
            glVertex3f(math.cos(angle1) * PIT_RADIUS, 0.5, math.sin(angle1) * PIT_RADIUS)

            glColor3f(r, g, b)
            glVertex3f(math.cos(angle2) * PIT_RADIUS, 0.5, math.sin(angle2) * PIT_RADIUS)
            glEnd()
...
def draw_first_person_overlay():
    ...
    for angle in range(0, 360, 10):
        angle_rad = math.radians(angle)
        x = math.cos(angle_rad) * 15
        y = math.sin(angle_rad) * 15 + 5  # Slightly above center
        glVertex3f(x, y, -40)
    glEnd()
...
def draw_health_bar(x, y, width, height):
    """Draw a visual health bar with background, border, and colored health"""
    # Calculate health percentage
    health_percentage = max(0, min(1, player_health / 100.0))
    ...
    if player_health > 75:
        ...
        glColor3f(0.5, 1.0, 0.0)  # Yellow-green
    ...
    if player_health < 25:
        pulse = 0.5 + 0.5 * (1.0 + math.sin(time.time() * 8.0))  # Fast pulse
        glColor3f(1.0, pulse * 0.3, pulse * 0.3)  # Pulsing red
    ...
    # Damage segments (show as darker lines every 10 health points)
    glColor3f(0.3, 0.3, 0.3)
    glLineWidth(1.0)
    for i in range(1, 10):  # 9 division lines for 10 segments
        segment_x = x + (width * i / 10)
        glBegin(GL_LINES)
        glVertex2f(segment_x, y)
        glVertex2f(segment_x, y + height)
        glEnd()
    
    # Health text overlay (centered in the bar)
    glColor3f(1.0, 1.0, 1.0)
    text_x = x + width // 2 - 15  # Approximate centering
    text_y = y + height // 2 - 4   # Center vertically
    glRasterPos2f(text_x, text_y)
    health_display = f"{player_health}"
    for char in health_display:
...
def draw_shield():
    ...
    # Use our global rotation
    glRotatef(shield_rotation, 1, 1, 1)
    ...
    for angle in range(0, 360, 360 // slices):
        ...
        for theta in range(-90, 91, 10):
            ...
            x = SHIELD_RADIUS * math.cos(t) * math.cos(rad)
            y = SHIELD_RADIUS * math.sin(t)
            z = SHIELD_RADIUS * math.cos(t) * math.sin(rad)
            glVertex3f(x, y, z)
        glEnd()
